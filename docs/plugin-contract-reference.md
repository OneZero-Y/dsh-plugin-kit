...
**Before relying on this pattern, confirm your target DSH version does not have the pre-`rc.7` settings-namespace allowlist.** Confirmed by building and shipping a real third-party client-half plugin: on `@deepseek-ai/dsh-client-ui-theme` `0.1.0-rc.6` and earlier, the official Host's `packages/host/apiproxy` gated every registered settings namespace behind a hardcoded `WEB_SETTINGS_NAMESPACES` allowlist that a third-party plugin's own repository could never join — a namespace outside it answered `settings-not-exposed` regardless of whether the host half above registered it correctly. Code built exactly as described here **compiles and runs with no error on that version, but the write silently never round-trips** — `ctx.settingsScope.bind(...).set(...)` resolves normally, and the very next page load reads back the pre-write value, because the namespace was never actually exposed to the browser at all. This allowlist was removed in the official repository's `2026-08-12-plugin-owned-settings-surface` Agent Note ("Registering is exposing"), first shipping in `rc.7`. If you cannot confirm your target version postdates that change, do not treat a successful build and a non-throwing `set()` call as proof the persistence path works — boot against a real profile, write a value, reload the page, and confirm the value survived, per "Proving activation for a client half" in `dsh-forge-wire`.
...
**An `allowBuilds` entry for a git spec is pinned to the exact commit, not to the package** — confirmed by re-installing a real plugin after pushing a new commit to its `main` branch. The key pnpm prints (and the one the user adds to `pnpm-workspace.yaml`) embeds the tarball URL for the specific commit resolved at that `add` time (`.../tar.gz/<sha>`); pushing a new commit changes that URL, so a profile's existing `allowBuilds` entry from a previous install stops matching and `add` fails again with a **new** key to add, even though the package name is unchanged. This is not a one-time setup step per package — document it as "expect a new `allowBuilds` prompt after every commit that changes what's installed," not as something a user configures once.
...
Two additional facts were confirmed the same way, against a second real third-party client-half plugin carried across a `dsh-client-ui-theme` `rc.6` → `rc.7` upgrade: the pre-`rc.7` settings-namespace allowlist (`WEB_SETTINGS_NAMESPACES` in the official `packages/host/apiproxy`) silently defeats `ctx.settingsScope`-based persistence with no error at any layer, fixed upstream by the official repository's `2026-08-12-plugin-owned-settings-surface` Agent Note; and an `allowBuilds` entry for a git-spec install is pinned to the resolved commit's tarball URL, not to the package name, so it goes stale on every new commit rather than being a one-time setup step.
